...
import {deleteTypeOrder, getTypeOrders} from '../../../../../http/typeOrderAPI'
import {observer} from 'mobx-react-lite'

const TableTypesOrder = observer(() => {
    // eslint-disable-next-line no-unused-vars
    const [typeOrderItems, setTypeOrderItems] = useState([]);
    ...
    useEffect(() => {
        getTypeOrders().then(data => setTypeOrderItems(data))
    }, [reloadPage])

    const openTypeOrder = (id) => {
        return navigate(`/admin/typeorders/show/${id}`);
    }

    const editTypeOrder = (id) => {
        return navigate(`/admin/typeorders/edit/${id}`);
    }

    const deleteTypeOrderItem = async (id) => {
        await deleteTypeOrder(id);
        ...
        <section className="bg-white dark:bg-gray-900">
            <div className="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6">
                <div className="mx-auto max-w-screen-sm">
                    <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">Типы заказов</h2>
                    <p className="mb-8 font-light text-gray-500 lg:mb-16 sm:text-xl dark:text-gray-400">Здесь можно увидеть все типы</p>
                </div>
                            ...
                            <div
                                ...
                                <div className="flex items-center flex-1 space-x-4">
                                    <h5>
                                        ...
                                        <span className="dark:text-white">{typeOrderItems.length}</span>
                                    </h5>
                                </div>
                                <div
                                    className="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                                    <button type="button" onClick={() => navigate('/admin/typeorders/create')}
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" className="px-4 py-3">Распознавание</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    {typeOrderItems.slice(paginateBack, paginateForward).map((i) => {
                                        return <tr key={i.id} className="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                            <td className="w-4 px-4 py-3">
                                                <div className={'flex items-center gap-2'}>
                                                    <button type="button" onClick={() => openTypeOrder(i.id)}
                                                    ...
                                                    <button type="button" onClick={() => editTypeOrder(i.id)}
                                                            className="px-2 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                                                        <FontAwesomeIcon icon={faEdit}/>
                                                    </button>
                                                    <button type="button" onClick={() => deleteTypeOrderItem(i.id)}
                                                            className="px-2 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                                ...
                                <div className={'flex items-center justify-center mt-4'}>
                                    ...
                                    <button type={'button'} onClick={() => {
                                        ...
                                        setPaginateForward(paginateForward + 5)
                                            ...
                                            disabled={typeOrderItems.slice(paginateBack, paginateForward).length < 5 ? true : false}
                                            className={`${typeOrderItems.slice(paginateBack, paginateForward).length < 5 ? 'hidden' : 'block'} inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white`}>
                                        Следующий
...
export default TableTypesOrder;