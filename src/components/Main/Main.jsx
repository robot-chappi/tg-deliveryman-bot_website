                    ...
                    <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
                        {main.tariffs.map(i => {
                            return <div key={i.id}
                                ...
                                <h3 className="mb-4 text-2xl font-semibold">{i.title}</h3>
                                <p className="font-light text-gray-500 sm:text-lg dark:text-gray-400">{i.description}</p>
                                <div className="flex justify-center items-baseline my-8">
                                    <span className="mr-2 text-5xl font-extrabold">{i.price}₽</span>
                                    <span className="text-gray-500 dark:text-gray-400">/ в месяц</span>
                                </div>

                                <ul className="mb-8 space-y-4 text-left">
                                    {i.privileges.map(p => {
                                        return <li key={p.id} className="flex items-center space-x-3">
