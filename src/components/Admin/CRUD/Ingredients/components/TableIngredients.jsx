...
import {deleteIngredient, getIngredients} from '../../../../../http/ingredientsAPI'
import {observer} from 'mobx-react-lite'

const TableIngredients = observer(() => {
    ...
    const openIngredient = (id) => {
        return navigate(`/admin/ingredients/show/${id}`);
    }

    const editIngredient = (id) => {
        return navigate(`/admin/ingredients/edit/${id}`);
    }

    const deleteIngredientItem = async (id) => {
        ...
        <section className="bg-white dark:bg-gray-900">
            <div className="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6">
                <div className="mx-auto max-w-screen-sm">
                    <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">Ингрединеты</h2>
                    <p className="mb-8 font-light text-gray-500 lg:mb-16 sm:text-xl dark:text-gray-400">Здесь можно увидеть все ингредиенты</p>
                </div>
                        ...
                        <div className="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                            <div
                                ...
                                <div className="flex items-center flex-1 space-x-4">
                                    <h5>
                                        <span className="text-gray-500">Всего ингредиентов: </span>
                                        <span className="dark:text-white">{ingredientItems.length}</span>
                                    </h5>
                                </div>
                                <div
                                    className="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                                    <button type="button" onClick={() => navigate('/admin/ingredients/create')}
                                        ...
                                        return <tr key={i.id} className="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                            <td className="w-4 px-4 py-3">
                                                <div className={'flex items-center gap-2'}>
                                                    ...
                                                    <button type="button" onClick={() => editIngredient(i.id)}
                                                            className="px-2 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                                                        <FontAwesomeIcon icon={faEdit}/>
                                                    </button>
                                                    <button type="button" onClick={() => deleteIngredientItem(i.id)}
                                                            className="px-2 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                                ...
                                <div className={'flex items-center justify-center mt-4'}>
                                    ...
                                    <button type={'button'} onClick={() => {
                                        ...
                                        setPaginateForward(paginateForward + 5)
                                            ...
                                            disabled={ingredientItems.slice(paginateBack, paginateForward).length < 5 ? true : false}
                                            className={`${ingredientItems.slice(paginateBack, paginateForward).length < 5 ? 'hidden' : 'block'} inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white`}>
                                        Следующий
